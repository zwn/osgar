...
#    rospy.Subscriber('/odom', Odometry, callback_odom)
    rospy.Subscriber('/excavator_1/joint_states', JointState, callback_odom)
...
#    rospy.Subscriber('/clock', Clock, callback_clock)
    ...
    rospy.Subscriber('/excavator_1/camera/left/image_raw/compressed', CompressedImage, callback_topic, 
                     '/excavator_1/camera/left/image_raw/compressed')
    rospy.Subscriber('/excavator_1/camera/right/image_raw/compressed', CompressedImage, callback_topic, 
                     '/excavator_1/camera/right/image_raw/compressed')
...
#    velocity_publisher = rospy.Publisher('/cmd_vel', Twist, queue_size=10)
    ...
    QSIZE = 10
    vel_fl_publisher = rospy.Publisher('/excavator_1/fl_wheel_controller/command', Float64, queue_size=QSIZE)
    vel_fr_publisher = rospy.Publisher('/excavator_1/fr_wheel_controller/command', Float64, queue_size=QSIZE)
    vel_bl_publisher = rospy.Publisher('/excavator_1/bl_wheel_controller/command', Float64, queue_size=QSIZE)
    vel_br_publisher = rospy.Publisher('/excavator_1/br_wheel_controller/command', Float64, queue_size=QSIZE)

    steering_fl_publisher = rospy.Publisher('/excavator_1/fl_steering_arm_controller/command', Float64, queue_size=QSIZE)
    steering_fr_publisher = rospy.Publisher('/excavator_1/fr_steering_arm_controller/command', Float64, queue_size=QSIZE)
    ...
    while True:
        try:
            ...
            if message_type == "cmd_rover":
                ...
                try:
                    rospy.wait_for_service("/excavator_1/get_true_pose", timeout=2.0)
                    request_origin = rospy.ServiceProxy('/excavator_1/get_true_pose', LocalizationSrv)
                    p = request_origin(True)
                    s = "origin excavator_1 %f %f %f  %f %f %f %f" % (p.pose.position.x, p.pose.position.y, p.pose.position.z, 
